...
const OrderHistory = () => {
  ...
  const getStatusBadgeClass = (status) => {
    switch (status) {
      case 'Payment Completed': return 'bg-success';
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td className="px-4 py-3">
                    <ul className="list-unstyled m-0 p-0" style={{ listStyleType: 'none' }}>
                      {order.OrderHistoryItems?.map((item, index) => (
                        <li key={index} className="d-flex align-items-center mb-3">
                          ...
                          <div className="flex-grow-1 d-flex flex-column justify-content-center">
                            <span className="fw-medium text-dark mb-0" style={{ fontSize: '0.9rem', lineHeight: '1.2' }}>
                              {item.Product?.product_description || "No product description"}
                            </span>
                            <div className="d-flex align-items-center mt-1">
                              <span className="badge bg-secondary-subtle text-secondary border rounded-pill px-2 py-1" style={{ fontSize: '0.75rem' }}>
...
const cellStyle = { padding: '15px 20px' };
